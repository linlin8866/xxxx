...
ssl_cert_issue() {
    ...
    if [[ "$setReloadcmd" == "y" || "$setReloadcmd" == "Y" ]]; then
        ...
        case "$choice" in
            1) reloadCmd="systemctl reload nginx ; systemctl restart x-ui" ;;
            2) read -rp "Please enter your custom reloadcmd: " reloadCmd ;;
            *) ;;
        esac
    fi
    local installOutput=""
...
install_x-ui() {
    ...
    # ========== 完全本地使用 /root/x-ui-linux-amd64.tar.gz ==========
    if [[ ! -f "${LOCAL_TAR}" ]]; then
        echo -e "${red}Error: 本地压缩包不存在 ${LOCAL_TAR}${plain}"
        echo -e "${yellow}请先把 x-ui-linux-amd64.tar.gz 上传到 /root 目录${plain}"
        exit 1
    fi
    # 复制本地压缩包到当前目录
    cp -f "${LOCAL_TAR}" ./x-ui-linux-amd64.tar.gz

    # 下载 x-ui.sh 控制脚本（如需完全断网我再帮你改成本地读取）
    ...
    echo -e "${green}x-ui local offline installation finished, it is running now...${plain}"
    echo -e ""
    echo -e "┌───────────────────────────────────────────────────────┐
...
echo -e "${green}Running offline local install...${plain}"
install_base
install_x-ui $1
